...
-- Auto Summon (rolls only -- still needs a brief teleport to the plot's
-- Summon prompt; verified live the Roll remote can't be fired remotely,
-- only the real ProximityPrompt actually generates a new roll) + Auto Buy
-- (event-driven off the roll result itself: price range, keep-gold reserve,
-- per-rarity enable + per-rarity Character/Mutation filters -- verified this
-- part genuinely works from any distance, no teleport) + Anti-AFK +
-- Auto Reconnect + Auto Save
...
-- Internal key is "Slots" (plural), displayed as "Slot" per request.
local UPGRADE_KEYS = {"Gold", "Luck", "Slots", "Inventory"}
local UPGRADE_LABELS = {Gold = "Gold", Luck = "Luck", Slots = "Slot", Inventory = "Inventory"}

-- ===== Character/Rarity/Mutation catalog =====
-- CharactersInfo.Colors' keys are the game's real rarity tiers (verified
-- live) -- matches exactly. CharactersInfo.Characters is a flat array of
-- {Name, Rarity, Price, ...combat stats}; Mutation isn't in that table, it's
-- set as a live attribute on the rolled instance (SetAttribute("Mutation", ...))
-- or absent entirely for a plain roll, hence "None" here.
...
local CHAR_INFO = {}
local CHAR_BASE_STATS = {} -- full CharactersInfo entry per name, for StatsHandler.GetStats(base, unit, nil, nil)
...
for _, r in ipairs(RARITIES) do table.sort(RARITY_CHARACTERS[r]) end
...
if getgenv().SummonDelay == nil then getgenv().SummonDelay = 2.2 end -- matches the game's own normal roll debounce
...
-- Verified live: your plot is tagged with an Owner attribute matching your
-- display name (workspace.Plots.Plot<N>.Owner == LocalPlayer.Name).
...
-- ===== Auto Summon (rolls only -- buying is Auto Buy's job now) =====
-- Roll:FireServer() (no args) does NOT generate a new roll -- verified live
-- it only re-syncs your last-committed roll result (6+ calls in a row, at
-- various delays, all returned the exact same character). Rolling genuinely
-- requires the real ProximityPrompt, which Roblox validates server-side for
-- actual distance -- confirmed the same call from ~100 studs away produces
-- nothing, while triggering it up close produces genuinely varied results
-- every time. So teleporting for the roll step specifically can't be
-- avoided; only the buy step (below) can skip it.
...
local function doAutoSummon()
    ...
    if not plot then
        summonStatusText = "Could not find your plot"
        return
    end
    local prompt = getRollPrompt(plot)
    if not prompt then
        summonStatusText = "Could not find the Summon prompt"
        return
    end
    local hrp = getHRP()
    ...
    pcall(function()
        hrp.CFrame = CFrame.new(promptPart.Position + Vector3.new(0, 3, 3))
    end)
    task.wait(0.2)
...
-- ===== Auto Buy =====
-- Event-driven, not polling: Roll.OnClientEvent delivers the full roll
-- result (rollId + per-slot {Name, Rarity, Price, Mutation, Purchased}) the
-- instant a roll happens -- whether triggered by Auto Summon above or a
-- manual roll. Buy:FireServer(rollId, slotIndex) also verified live to work
-- from any distance, so this needs no character-instance lookup, no
-- BuyPrompt polling, and no teleporting -- it fires directly off the event
-- data. Gated ONLY by AutoBuyEnabled, independent of AutoSummonEnabled, so
-- having Auto Summon on by itself never buys anything.
...
-- Connected once, persists across script reloads (same guard pattern as
-- Anti-AFK/Auto Reconnect below) -- reads getgenv() for settings and the
-- current window's status label via getgenv().__RABuyStatusLabel, so an old
-- connection from a previous generation still targets the live UI correctly.
...
-- ===== Auto Upgrade =====
-- Reads live level via DataClient:get({"Upgrades", key}) (falls back to
-- UpgradesInfo.Upgrades.Default[key] the same way the game's own Upgrade UI
-- does), buys with Upgrade:FireServer("Gold", key) -- "Gold" here is the
-- payment method (as opposed to the separate Robux-priced button), not the
-- upgrade category. Each category has its own toggle so any subset can run
-- independently; a small per-key persistent debounce avoids re-firing the
-- same purchase every loop tick before the server's response updates the
-- cached level.
...
-- ===== Fight Control =====
-- Fight.Start (UnreliableRemoteEvent) takes a plain action string -- verified
-- live in the game's own FightClient: FireServer("Start")/("Stop"). Wave
-- progress is LocalPlayer:GetAttribute("FightWave") (server-driven, verified
-- live == 44 mid-fight), and whether a fight is currently running is
-- plot:GetAttribute("FightRunning"). The remote takes no wave argument and a
-- Stop+Start cycle resets wave back to 1, so "Start at wave" can only gate
-- Auto Start until the wave counter has naturally reached that value at
-- least once (bootstrap by starting manually first).
...
local function doAutoFight()
    ...
    if running then
        fightStatusText = "Fighting (wave " .. wave .. ")"
        return
    end
...
-- ===== Auto Equip Best =====
-- Reads client:get("Inventory") (owned units: UUID/Name/Level/Mutation/
-- Equipped), scores each with the game's own StatsHandler.GetStats(base,
-- unit, nil, nil) using its real base stats from CharactersInfo -- verified
-- live this returns real scaled Attack/Health (e.g. Jiren Lv1: Atk=500,
-- HP=16500 vs Zoro Lv1: Atk=30, HP=320). Available slot count is the raw
-- "Slots" upgrade value itself (not a display level -- verified live raw=7
-- while the UI shows "Lv.4", since Slots counts up from a base of 4).
-- UpdateInventory:FireServer("ToggleHotbar", uuid) toggles a unit in/out --
-- verified this exact call is what the game's own per-card equip button
-- fires. IMPORTANT, verified live: it (and even the native one-click
-- "EquipBest" button's own remote call) has NO effect while a fight is
-- currently running (plot:GetAttribute("FightRunning") == true) -- the
-- server silently ignores roster changes mid-fight, same as a real player
-- couldn't do it either. So this only acts between fights.
local equipStatusText = "Idle"
local function doAutoEquipBest()
    if not getgenv().AutoEquipBestEnabled then
        equipStatusText = "Idle"
        return
    end
    local plot = getMyPlot()
    ...
    if getgenv().ReplaceWeakerSlots then
        local targetSet = {}
        for i = 1, math.min(maxSlots, #scored) do
            targetSet[scored[i].unit.UUID] = true
        end
        ...
        for _, s in ipairs(scored) do
            if s.unit.Equipped then equippedCount += 1 end
        end
...
-- ===== Auto Spin Wheel =====
-- client:get({"Spin"}) is your free-spin ticket count; Spin:FireServer("Spin")
-- consumes one (matches the game's own requestSpin(), which guards the same
-- way). The recharge countdown uses the exact formula from the game's own
-- SpinWheelClient (SpinWheelHandler.Config.Restart hours since SpinLastClaim
-- / SpinRechargeAt), verified live (restartHours=4).
...
local function doAutoSpinWheel()
    ...
    if getgenv().AutoSpinWheelEnabled then
        ...
        if spinCount > 0 then
            pcall(function() SpinRemote:FireServer("Spin") end)
        end
    end
end

-- ===== Performance =====
-- FPSBoost is a real native LocalPlayer attribute -- verified live the
-- game's own FightClient checks `LocalPlayer:GetAttribute("FPSBoost")`
-- directly to lower visual effects, so just setting it does the same thing
-- the game's own settings menu would.
local function applyFPSBoost(enabled)
    pcall(function() LocalPlayer:SetAttribute("FPSBoost", enabled) end)
end

-- Detaches other players' plot models from the client's copy of the
-- Workspace tree (Parent = nil) -- purely local/visual, reversible by
-- re-parenting back to workspace.Plots, doesn't touch the server or other
-- players. Does not touch your own plot (matched the same way as getMyPlot()).
...
local function applyRemoveOtherBase(enabled)
    ...
    if enabled then
        ...
        for plot in pairs(otherPlotsDetached) do
            pcall(function() plot.Parent = plots end)
        end
        otherPlotsDetached = {}
    end
end
...
-- Tab icons use Roblox's Lucide icon pack (rbxassetid images), verified
-- against dawid-scripts/Fluent's src/Icons.lua (master branch).
local TabGroup = Window:TabGroup()
local Tabs = {
    Info = TabGroup:Tab({Name = "Info", Image = "rbxassetid://10723415903"}),         -- lucide-info
    Summon = TabGroup:Tab({Name = "Summon", Image = "rbxassetid://10723343321"}),     -- lucide-dices
    Buy = TabGroup:Tab({Name = "Buy", Image = "rbxassetid://10734952273"}),           -- lucide-shopping-bag
    Fight = TabGroup:Tab({Name = "Fight", Image = "rbxassetid://10734975692"}),       -- lucide-swords
    Upgrade = TabGroup:Tab({Name = "Upgrade", Image = "rbxassetid://10747363465"}),   -- lucide-trending-up
...
-- ----- Info Tab (4 separate boxes: each Section() call is its own bordered box) -----

-- Box 1: User Info
local InfoBox1 = Tabs.Info:Section({Side = "Left"})
InfoBox1:Header({Text = "User Info"})
InfoBox1:Label({Text = "User : " .. LocalPlayer.Name})
InfoBox1:Label({Text = "Status : Free"}) -- no paid-tier logic yet, hardcoded per request
...
InfoBox3:Label({Text = "* Auto Buy (price range, keep-gold, per-rarity\n  Character/Mutation rules)"})
...
InfoBox4:Button({
    Name = "Copy Rscripts URL",
    Callback = function()
        pcall(function() setclipboard("https://rscripts.net/") end)
    end,
})
InfoBox4:Label({Text = "(Rscripts link is a placeholder -- tell me the\nexact page and I'll swap it in.)"})
...
SummonSection:Slider({
    ...
    Minimum = 0.5,
    Maximum = 3.0,
    Default = getgenv().SummonDelay,
    Precision = 2,
    Callback = function(value) getgenv().SummonDelay = value; saveState() end,
})
SummonSection:Label({Text = "Rolls only -- still needs a brief teleport to the\nSummon prompt (the roll itself can't be done remotely).\nBuying is fully remote though -- see the Buy tab.\nGame's own debounce: ~0.6s (Fast Summon) / ~2.2s (normal)."})
...
SummonBox2:Toggle({
    Name = "Replace Weaker Slots",
    Default = getgenv().ReplaceWeakerSlots,
    Callback = function(value) getgenv().ReplaceWeakerSlots = value; saveState() end,
}, "ReplaceWeakerSlots")
SummonBox2:Label({Text = "OFF: only fills empty slots, never unequips.\nON: swaps out weaker equipped units too."})
local equipStatusLabel = SummonBox2:Label({Text = "Idle"})

-- Box 3: Auto Spin Wheel
local SummonBox3 = Tabs.Summon:Section({Side = "Right"})
SummonBox3:Header({Text = "Auto Spin Wheel"})
SummonBox3:Toggle({
    Name = "Auto Spin Wheel",
    Default = getgenv().AutoSpinWheelEnabled,
    Callback = function(value) getgenv().AutoSpinWheelEnabled = value; saveState() end,
}, "AutoSpinWheelEnabled")
SummonBox3:Label({Text = "Only uses free spin tickets -- never buys\npaid (Robux) spins."})
...
BuyLeft:Toggle({
    Name = "Auto Buy Summon",
    Default = getgenv().AutoBuyEnabled,
    Callback = function(value) getgenv().AutoBuyEnabled = value; saveState() end,
}, "AutoBuyEnabled")
BuyLeft:Input({
    Name = "Min Price (0 = any)",
    Placeholder = tostring(getgenv().MinPrice),
    AcceptedCharacters = "Numeric",
    Callback = function(text) getgenv().MinPrice = tonumber(text) or 0; saveState() end,
}, "MinPriceInput")
BuyLeft:Input({
    Name = "Max Price (0 = any)",
    Placeholder = tostring(getgenv().MaxPrice),
    AcceptedCharacters = "Numeric",
    Callback = function(text) getgenv().MaxPrice = tonumber(text) or 0; saveState() end,
}, "MaxPriceInput")
BuyLeft:Input({
    Name = "Keep Gold (0 = none)",
    Placeholder = tostring(getgenv().KeepGold),
    AcceptedCharacters = "Numeric",
    Callback = function(text) getgenv().KeepGold = tonumber(text) or 0; saveState() end,
}, "KeepGoldInput")
...
RaritySection:Label({Text = "Pick which rarities to buy. Selecting one reveals\nits Character/Mutation filters -- leave those empty\nto allow any character/mutation of that rarity."})
...
for _, r in ipairs(RARITIES) do
    local charOptions = RARITY_CHARACTERS[r]
    local charDD = RaritySection:Dropdown({
        Name = r .. " Character",
        Multi = true,
        Search = true,
        Options = charOptions,
        Default = getgenv().RarityRules[r].characters,
        Callback = function(value)
            local selectedSet = {}
            if type(value) == "table" then
                for k, v2 in pairs(value) do
            ...
            local newList = {}
            for _, name in ipairs(charOptions) do
...
FightSection:Toggle({
    Name = "Auto Start",
    Default = getgenv().AutoFightStart,
    Callback = function(value) getgenv().AutoFightStart = value; saveState() end,
}, "AutoFightStart")
FightSection:Input({
    Name = "Start at wave",
    Placeholder = tostring(getgenv().StartAtWave),
    AcceptedCharacters = "Numeric",
    Callback = function(text) getgenv().StartAtWave = tonumber(text) or 0; saveState() end,
}, "StartAtWaveInput")
FightSection:Input({
    Name = "Stop at wave",
    Placeholder = tostring(getgenv().StopAtWave),
    AcceptedCharacters = "Numeric",
    Callback = function(text) getgenv().StopAtWave = tonumber(text) or 0; saveState() end,
}, "StopAtWaveInput")
FightSection:Toggle({
    Name = "Max Game Speed",
    Default = getgenv().MaxGameSpeedEnabled,
    Callback = function(value) getgenv().MaxGameSpeedEnabled = value; saveState() end,
}, "MaxGameSpeedEnabled")
FightSection:Label({Text = "0 = disabled for both. Start at wave: Auto Start\nwon't fire until wave has reached this at least\nonce (start manually first to bootstrap). Stop at\nwave: auto-stops when reached. Max Game Speed\ncycles the native FastForward to its 3x cap\n(can't exceed the game's own limit)."})
...
for _, key in ipairs(UPGRADE_KEYS) do
    UpgradeSection:Toggle({
        ...
        Callback = function(value) getgenv()[UPGRADE_FLAG_KEYS[key]] = value; saveState() end,
...
PerformanceSection:Toggle({
    Name = "Remove Other Base",
    Default = getgenv().RemoveOtherBaseEnabled,
    Callback = function(value)
        getgenv().RemoveOtherBaseEnabled = value
        applyRemoveOtherBase(value)
        saveState()
    end,
}, "RemoveOtherBaseEnabled")
PerformanceSection:Label({Text = "Remove Other Base is local-only (client-side) --\nother players still see everything normally."})
...
-- Auto Buy has no loop of its own -- it's driven entirely by the persistent
-- Roll.OnClientEvent hook above, which fires the instant a roll happens.

task.spawn(function()
    while getgenv().__RAG == myGen do
        pcall(function()
            goldStatusLabel:UpdateName("Gold: " .. tostring(getGold()))
        end)
...
task.spawn(function()
    while getgenv().__RAG == myGen do
        pcall(function()
            sessionTimeLabel:UpdateName("SessionTime : " .. formatSessionTime())
        end)
        task.wait(1)
    end
end)
...
-- Apply persisted Performance settings on load (not just on toggle click)
